...
let solve line = 
  ...
  let rec count x y x1 y1 set pattern idx =
    match pattern with
    | [] -> SS.cardinal set
    | head :: tail ->
      if idx mod 2 = 0 then 
        ...
        count x y x1 y1 set tail (idx+1)
      else 
        let x1, y1 =
        match head with
        | '^' -> (x1 - 1, y1)
        ...
        | '<' -> (x1, y1 - 1)
        | '>' -> (x1, y1 + 1)
        | _ -> (x1, y1)  (* Handle other characters as needed *)
      in 
      let set = SS.add (get_key x1 y1) set in
      count x y x1 y1 set tail (idx+1)
    in
